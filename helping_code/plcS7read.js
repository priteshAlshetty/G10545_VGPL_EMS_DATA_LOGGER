...
async function readPLCs7DB( {pool, client, plc_ip, rack, slot, DB_number, meter_size, 
    ...
    async function dataEntries(meterName, meterData){

        if(!allowed_tables.has(meterName)){
            throw new Error('Invalid table name:', meterName)
        }

        try{
            const sql = `
                INSERT INTO ${meterName} (
            ...
            //result.affected rows>0 --> success else throw error
    ...
    async function readPLCDB(){
        return new Promise((resolve, reject) => {
           const result = client.DBRead(DB_number, 0, total_bytes, (err, data) => {
    ...
    async function processMeters(buf) {
        ...
        for (let i = 0; i < meter_count; i++) {
            ...
            await dataEntries(meterNames[i], meter_data);
        }
    }
    ...
    async function startPolling(){
        ...
        pollTimer = setInterval(async () => {
            try{
                ...
                await processMeters(buf);
                console.log('PLC Data inserted...');
    ...
    process.on('SIGINT', () => {
        console.log('Ctrl+C pressed, but process will shutdown.');
        stopPolling();
        client.Disconnect();
        pool.end();
        process.exit(0);
...
await readPLCs7DB( {pool: pool, client: client, plc_ip: '192.168.0.205', rack: 0, slot: 1, 
